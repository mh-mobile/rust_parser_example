...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
       ...
       match self {
           ...
           UnclosedOpenParen(tok) => write!(f, "{}: exppression after '{}' is redundant", tok.loc, tok.value),
...
impl StdError for InterpreterError {
    ...
    use self::InterpreterErrorKind::*;
    match self.value {
        DivisionByZero => "the right hand expression of the division evaluate to zero",
    } 
    ...
    pub fn eval(&mut self, expr: &Ast) -> Result<i64, InterpreterError> {
        ...
        match expr.value {
            ...
            BinOp {
                ...
                let l = self.eval(l)?;
                let r = self.eval(r)?;
                self.eval_binop(op, l, r).map_err(|e| InterpreterError::new(e, expr.loc.clone()))
            }
        }
    }
...
impl Ast {
    ...
    fn binop(op: BinOp, e: Ast, r: Ast, loc: Loc) -> Self {
        Self::new(
            AstKind::BinOp {
                op,
                l: Box::new(e),
